...
class TestModernAdminRecheckUserCommand:
    ...
    def mock_bot(self):
        ...
        bot.config = {
            'channels': {
                'leadership_announcement_channel_id': 999999999
            }
        }
    ...
    @pytest.mark.asyncio
    async def test_recheck_user_no_change_scenario(self, admin_cog, mock_interaction, mock_member):
        ...
        with patch('services.db.database.Database.get_connection') as mock_db:
            ...
            mock_db.return_value.__aenter__.return_value.execute.return_value = mock_cursor

            # Mock snapshot and diff
            with patch('helpers.snapshots.snapshot_member_state'), \
                ...
                with patch('helpers.role_helper.reverify_member') as mock_reverify:
                    ...
                    with patch('helpers.task_queue.flush_tasks'):
                        ...
                        with patch('helpers.leadership_log.post_if_changed'):
                            ...
                            with patch('helpers.announcement.send_admin_recheck_notification') as mock_admin_notif:
                                mock_admin_notif.return_value = True

                                # Mock bulk announcer - should NOT be called for no change
                                with patch('helpers.announcement.enqueue_verification_event') as mock_bulk_enqueue:
                                    ...
                                    assert "Main" in response_msg  # Should show the unchanged status

    @pytest.mark.asyncio
    async def test_recheck_user_status_change_scenario(self, admin_cog, mock_interaction, mock_member):
                        ...
                        with patch('helpers.leadership_log.post_if_changed'):
                            ...
                            with patch('helpers.announcement.send_admin_recheck_notification') as mock_admin_notif:
                                ...
                                with patch('helpers.announcement.enqueue_verification_event') as mock_bulk_enqueue:
                                    ...
                                    assert "Affiliate" in response_msg and "Main" in response_msg  # Should show both statuses

    @pytest.mark.asyncio
    async def test_recheck_user_unknown_statuses_fallback(self, admin_cog, mock_interaction, mock_member):
            ...
            with patch('helpers.snapshots.snapshot_member_state'), \
                ...
                with patch('helpers.role_helper.reverify_member') as mock_reverify:
                    ...
                    with patch('helpers.task_queue.flush_tasks'), \
                         ...
                         patch('helpers.announcement.send_admin_recheck_notification') as mock_admin_notif, \
                         patch('helpers.announcement.enqueue_verification_event') as mock_bulk_enqueue:
    ...
    @pytest.mark.asyncio
    async def test_recheck_user_permission_denied(self, admin_cog, mock_interaction, mock_member):
    ...
    @pytest.mark.asyncio
    async def test_recheck_user_not_verified(self, admin_cog, mock_interaction, mock_member):
