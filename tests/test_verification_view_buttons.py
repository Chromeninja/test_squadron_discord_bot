...
async def test_verification_view_buttons_callbacks(monkeypatch, mock_bot) -> None:
    ...
    async def spy_get(interaction) -> None:
        called["get_token"] = True
        # Patch internals to avoid network/DB
        monkeypatch.setattr(
            "helpers.views_verification.check_rate_limit",
            AsyncMock(return_value=(False, 0)),
        )
        monkeypatch.setattr(
            "helpers.views_verification.log_attempt", AsyncMock(return_value=None)
        )
    ...
    await view.get_token_button.callback(ix)  # type: ignore[arg-type]
    await view.verify_button.callback(ix)  # type: ignore[arg-type]
