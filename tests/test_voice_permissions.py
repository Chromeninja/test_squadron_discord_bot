...
from helpers.voice_permissions import assert_base_permissions, enforce_permission_changes
from helpers.voice_utils import get_user_channel


@pytest.mark.asyncio
async def test_assert_base_permissions():
...
@pytest.mark.asyncio
async def test_enforce_permission_changes():
    ...
    with patch('helpers.voice_permissions.assert_base_permissions', new=AsyncMock()) as mock_assert:
        # Call the function
        ...
        mock_assert.assert_called_once_with(channel, bot_member, owner_member, default_role)
