...
class TestVoiceOwnerCommand:
    ...
    def mock_interaction(self, mock_guild):
        ...
        interaction.user.roles = [
            MagicMock(id=admin_role_id) for admin_role_id in [111, 222]
        ]
        return interaction
        ...
        try:
            ...
            with patch.object(
                ...
                await voice_commands.list_owners.callback(
                    voice_commands, mock_interaction
                )
    ...
    async def test_voice_owner_no_channels(
        ...
        with patch.object(
            ...
            if call_args[0]:  # If there are positional args
                ...
                "embed" in call_args[1]
            ):  # If it's an embed, check its fields for empty channels message
                embed = call_args[1]["embed"]
                # For empty list, check if there are no fields or if description suggests emptiness
                if not embed.fields:
                    message = (
                        "No managed voice channels found"  # Assume empty if no fields
                    )
                else:
    ...
    async def test_voice_owner_handles_missing_discord_objects(
        self, voice_commands, mock_interaction, mock_guild, temp_db
    ):
        ...
        with patch.object(
            ...
            if "embed" in call_args[1]:
                ...
                assert (
                    ...
                    or "Total: 1 channels" in embed.footer.text
                )
            else:
    ...
    @pytest.mark.asyncio
    async def test_voice_owner_cache_fallback(
        self, voice_commands, mock_interaction, mock_guild, temp_db
    ):
