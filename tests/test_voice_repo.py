...
@pytest.mark.asyncio
async def test_get_stale_voice_entries(test_db) -> None:
...
@pytest.mark.asyncio
async def test_cleanup_user_voice_data(test_db) -> None:
...
@pytest.mark.asyncio
async def test_get_user_channel_id(test_db) -> None:
...
@pytest.mark.asyncio
async def test_upsert_channel_settings(test_db) -> None:
    """Test upserting channel settings."""
    async with Database.get_connection() as db:
        await setup_test_data(db)

        # Update existing settings
        await upsert_channel_settings(
            1001, 1, 101, channel_name="Updated Channel", user_limit=15
        )
        ...
        # Insert new settings
        await upsert_channel_settings(
            2001, 1, 101, channel_name="New Channel", user_limit=20, lock=1
        )
...
@pytest.mark.asyncio
async def test_list_permissions(test_db) -> None:
...
@pytest.mark.asyncio
async def test_set_feature_row(test_db) -> None:
    """Test setting a feature row."""
    async with Database.get_connection() as db:
        await setup_test_data(db)

        # Set a new PTT setting
        await set_feature_row("channel_ptt_settings", 1001, 1, 101, 2001, "user", True)
        ...
        await set_feature_row("channel_ptt_settings", 1001, 1, 101, 2001, "user", False)
...
@pytest.mark.asyncio
async def test_transfer_channel_owner(test_db) -> None:
