...
class ConfigSchema:
    ...
    def validate_value(cls, key: str, value: Any) -> tuple[bool, str, Any]:
        ...
        try:
            if expected_type == bool:
                ...
                if isinstance(value, str):
                    if value.lower() in ("true", "1", "yes", "on", "enabled"):
                        ...
                        return False, "Invalid boolean value. Use 'true' or 'false'", None
                else:
                ...
                if isinstance(value, str):
                    try:
                        ...
                        if not isinstance(coerced_value, list):
                            return False, f"Expected a JSON array, got {type(coerced_value).__name__}", None
                    except json.JSONDecodeError as e:
                        return False, f"Invalid JSON array: {e}", None
                elif isinstance(value, list):
                    coerced_value = value
                else:
                    return False, f"Expected a JSON array or list, got {type(value).__name__}", None
                ...
                if "element_type" in config_info:
                    ...
                    try:
                        ...
                        return False, f"List contains invalid {element_type.__name__} values: {e}", None
            else:
                coerced_value = value
            ...
            return False, f"Cannot convert '{value}' to {expected_type.__name__}: {e}", None
...
class AdminCog(commands.Cog):
    ...
    async def reset_all(self, interaction: discord.Interaction) -> None:
        ...
        self.logger.info(f"'reset-all' command triggered by user {interaction.user.id}.")
    ...
    async def view_logs(self, interaction: discord.Interaction) -> None:
        ...
        self.logger.info(f"'view-logs' command triggered by user {interaction.user.id}.")
        ...
        try:
            ...
            if len(recent_content) > max_preview_length:
                preview_content = recent_content[:max_preview_length] + "... (preview truncated)"
            else:
                preview_content = recent_content + "\n(showing recent entries - full log sent via DM)"
            ...
            try:
                ...
                self.logger.warning(f"Could not DM full logs to admin {interaction.user.id}: {dm_error}")
            except Exception as e:
                # Other unexpected errors
                self.logger.warning(f"Failed to send DM to admin {interaction.user.id}: {e}")
            ...
            self.logger.exception(f"Error in view-logs command: {e}")
            await interaction.followup.send(
    ...
    async def recheck_user(
        ...
        try:
            ...
            self.logger.info(f"'recheck-user' command triggered by user {interaction.user.id} for member {member.id}.")
            ...
            try:
                ...
                self.logger.exception(f"Error during reverification: {e}")
                await interaction.followup.send(
            ...
            if not success:
                ...
                try:
                    ...
                    self.logger.warning(f"Could not DM admin about recheck failure: {dm_error}")
            ...
            try:
                ...
                if success and role_assignment_result:
                    ...
                    if isinstance(role_assignment_result, tuple | list) and len(role_assignment_result) >= 2:
                        old_status_raw, new_status_raw = role_assignment_result[0], role_assignment_result[1]
                    else:
                ...
                if status_changed:
                    await enqueue_verification_event(member, old_status_raw or "non_member", new_status_raw or "non_member")
                ...
                self.logger.warning(f"Failed to handle admin recheck announcements: {e}")
            ...
            self.logger.exception(f"Error in recheck-user command: {e}")
            await interaction.followup.send(
        ...
        try:
            ...
            try:
                voice_cooldown = await self.bot.services.config.get(
                    interaction.guild.id, "voice.cooldown_seconds",
                    default=60, parser=int
                )
            ...
            self.logger.exception(f"Error in guild-config command: {e}")
            await interaction.followup.send(
    ...
    async def key_autocomplete(
        ...
        for key, config_info in ConfigSchema.ALLOWED_KEYS.items():
            if not current or current.lower() in key.lower() or current.lower() in config_info["description"].lower():
                choices.append(app_commands.Choice(
                    name=f"{key} - {config_info['description']}",
                    value=key
                ))
    ...
    async def value_autocomplete(
        ...
        if not current:
            choices.append(app_commands.Choice(
                name=f"💡 {hint}",
                value=config_info.get("json_example", "")
            ))
        ...
        if expected_type == bool:
            ...
            if "prefix" in key:
                ...
                common_values = ["UTC", "America/New_York", "Europe/London", "Asia/Tokyo"]
            else:
                common_values = []
    ...
    async def set_config(
        ...
        try:
            ...
            is_valid, error_message, coerced_value = ConfigSchema.validate_value(key, value)
            ...
            await self.bot.services.config.set(
                interaction.guild.id, key, coerced_value
            )
            ...
            if expected_type == int:
                ...
                if element_type == int:
                    def parser(x):
                        return [int(i) for i in json.loads(x)] if isinstance(x, str) else x
                else:
                    def parser(x):
            ...
            self.logger.exception(f"Error in set-config command: {e}")
            await interaction.followup.send(
                "❌ **Configuration Error**\\n"
