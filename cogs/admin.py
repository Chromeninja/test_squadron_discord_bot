...
class Admin(commands.Cog):
        ...
        try:
            if self.bot.guilds:
                for guild in self.bot.guilds:
                    resolved_bot_admins, missing_bot_admins = (
                        resolve_role_ids_for_guild(guild, self.BOT_ADMIN_ROLE_IDS)
                    )
                    resolved_leads, missing_leads = resolve_role_ids_for_guild(
                        guild, self.LEAD_MODERATOR_ROLE_IDS
                    )
                    if missing_bot_admins:
                        logger.warning(
                            f"BOT_ADMIN_ROLE IDs missing in guild '{guild.name}' ({guild.id}): {missing_bot_admins}"
                        )
                    if missing_leads:
                        logger.warning(
                            f"LEAD_MODERATOR_ROLE IDs missing in guild '{guild.name}' ({guild.id}): {missing_leads}"
                        )
    ...
    async def status(self, interaction: discord.Interaction):
        ...
        await send_message(
            interaction,
            f"✅ Bot is online and operational. Uptime: {uptime}.",
            ephemeral=True,
        )
        logger.info(
    ...
    async def view_logs(self, interaction: discord.Interaction):
        ...
        try:
            if not os.path.exists(log_file_path):
                ...
                await send_message(
                    interaction, "ℹ️ No log file found yet.", ephemeral=True
                )
                return
            ...
            if len(logs) > 1900:
                await send_message(
                    interaction,
                    "ℹ️ Logs are too long to display here. Check your DM.",
                    ephemeral=True,
                )
                try:
                    ...
                    await send_message(
                        interaction,
                        "❌ I couldn't DM you the logs (DMs closed).",
                        ephemeral=True,
                    )
            else:
            ...
            logger.info(
                "View-logs command completed successfully.",
                extra={"user_id": interaction.user.id},
            )
        except Exception as e:
            logger.exception(
                f"Failed to send logs: {e}", extra={"user_id": interaction.user.id}
            )
            await send_message(
                interaction, "❌ Failed to retrieve logs.", ephemeral=True
            )
    ...
    async def admin_command_error(self, interaction: discord.Interaction, error):
        if isinstance(error, app_commands.errors.MissingAnyRole):
            ...
            logger.warning(
                f"Missing permissions for {interaction.command.name} by user {interaction.user.id}."
            )
            await send_message(
                interaction,
            ...
            logger.exception(
                f"Error in command {interaction.command.name}: {error}",
                extra={"user_id": interaction.user.id},
            )
            await send_message(
                interaction,
