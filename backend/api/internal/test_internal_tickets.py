...
# ---------------------------------------------------------------------------
# Regression Tests: Literal routes NOT shadowed by parameterized {ticket_id}
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_list_open_tickets_not_shadowed(
    async_client: AsyncClient, mock_repo: AsyncMock
) -> None:
    """Verify /tickets/open resolves to list_open_tickets, not coerced to int (422)."""
    mock_tickets = [
        {"id": 1, "guild_id": 123, "thread_id": 456, "user_id": 789, "status": "open"}
    ]
    mock_repo.get_open_tickets.return_value = mock_tickets

    async with async_client as client:
        response = await client.get(
            "/internal/guilds/123/tickets/open",
            headers={"X-Bot-Api-Key": VALID_KEY},
        )
    assert response.status_code == 200
    data = response.json()
...
@pytest.mark.asyncio
async def test_get_ticket_stats_not_shadowed(
    async_client: AsyncClient, mock_repo: AsyncMock
) -> None:
    """Verify /tickets/stats resolves to get_ticket_stats, not coerced to int (422)."""
    mock_stats = {"open": 5, "closed": 10, "total": 15}
    mock_repo.get_ticket_stats.return_value = mock_stats

    async with async_client as client:
        response = await client.get(
            "/internal/guilds/123/tickets/stats",
            headers={"X-Bot-Api-Key": VALID_KEY},
        )
    assert response.status_code == 200
    data = response.json()
    assert "stats" in data
    assert data["stats"]["total"] == 15


@pytest.mark.asyncio
async def test_get_thread_health_not_shadowed(
    async_client: AsyncClient, mock_repo: AsyncMock
) -> None:
    """Verify /tickets/thread-health resolves to get_thread_health, not coerced to int (422)."""
    mock_health = {"active": 10, "archived": 20, "deleted": 5}
    mock_repo.get_thread_health.return_value = mock_health

    async with async_client as client:
        response = await client.get(
            "/internal/guilds/123/tickets/thread-health",
            headers={"X-Bot-Api-Key": VALID_KEY},
        )
    assert response.status_code == 200
    data = response.json()
    assert "health" in data
    assert data["health"]["active"] == 10
