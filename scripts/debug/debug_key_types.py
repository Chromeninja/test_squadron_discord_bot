...
async def test_both_key_types():
    ...
    await config_service.set_guild_setting(guild_id, "test.setting", {"nested": "value"})
    ...
    retrieved_nested = await config_service.get_guild_setting(guild_id, "roles.admin", default="not found")
    print(f"Nested key result: {retrieved_nested}")

    # Test global setting method directly
    print("Testing global setting method...")
    global_nested = await config_service.get_global_setting("roles.admin", default="not found")
