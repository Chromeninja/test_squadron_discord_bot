...
async def migrate_roles(guild_id: int, config_path: str, dry_run: bool = False):
    ...
    if 'bot_admins' in roles and roles['bot_admins']:
        migrations.append(('roles.bot_admins', json.dumps(roles['bot_admins'])))
    
    if 'lead_moderators' in roles and roles['lead_moderators']:
        migrations.append(('roles.lead_moderators', json.dumps(roles['lead_moderators'])))
    
    if 'main_role' in roles and roles['main_role']:
    ...
    if 'affiliate_role' in roles and roles['affiliate_role']:
    ...
    if 'nonmember_role' in roles and roles['nonmember_role']:
    ...
    if dry_run:
        ...
        for key, value in migrations:
            print(f"   INSERT OR REPLACE INTO guild_settings (guild_id, key, value)")
