...
async def fetch_channel_settings(
    ...
    try:
        ...
        if user.voice and user.voice.channel:
            ...
            async with Database.get_connection() as db:
                ...
                if row:
                    ...
                    settings = await _get_all_user_settings(guild_id, jtc_channel_id, user.id)
                    if settings:
        ...
        if not result['is_active'] or allow_inactive:
            if target_user:
                ...
                for jtc_channel_id, settings in all_settings.items():
                    if settings:
                        embed = await _create_settings_embed(
                            user, settings, None, is_active=False, jtc_channel_id=jtc_channel_id
                        )
                        result['embeds'].append(embed)
        ...
        logger.exception(f"Error fetching channel settings: {e}")
        return result
...
async def _get_all_user_jtc_settings(
    ...
    try:
        ...
        logger.exception(f"Error getting all user JTC settings: {e}")
...
async def _create_settings_embed(
    ...
    if settings.get('lock'):
        basic_settings.append(f"**Lock:** {'🔒 Locked' if settings['lock'] else '🔓 Unlocked'}")
