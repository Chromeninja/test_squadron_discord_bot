...
async def assert_base_permissions(
    ...
    try:
        ...
        logger.error(f"Error asserting base permissions for channel {channel.id}: {e}")
...
async def enforce_permission_changes(
    ...
    try:
        ...
        if not owner_member:
            logger.error(f"Could not find owner member with ID {user_id} in guild {guild_id}")
            return
        ...
        logger.error(f"Error enforcing permission changes for channel {channel.id}: {e}")
