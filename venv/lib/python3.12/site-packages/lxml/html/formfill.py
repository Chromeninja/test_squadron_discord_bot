...
def fill_form_html(html, values, form_id=None, form_index=None) -> None:
...
def _takes_multiple(input) -> None:
...
def _fill_multiple(input, value) -> None:
    type = input.get('type', '').lower()
    if type == 'checkbox':
        v = input.get('value')
        if v is None:
...
def _fill_single(input, value) -> None:
    if _nons(input.tag) == 'textarea':
        input.text = value
    else:
        input.set('value', value)

def _find_form(el, form_id=None, form_index=None) -> None:
...
def insert_errors_html(html, values, **kw) -> None:
    result_type = type(html)
    if isinstance(html, basestring):
        doc = fromstring(html)
    else:
...
def _insert_error(el, error, error_class, error_creator) -> None:
...
def _add_class(el, class_name) -> None:
    if el.get('class'):
        el.set('class', el.get('class')+' '+class_name)
    else:
        el.set('class', class_name)

def _find_elements_for_name(form, name, error) -> None:
    if name is None:
