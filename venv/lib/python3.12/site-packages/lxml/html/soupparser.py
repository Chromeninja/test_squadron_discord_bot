...
def fromstring(data, beautifulsoup=None, makeelement=None, **bsargs) -> None:
...
def parse(file, beautifulsoup=None, makeelement=None, **bsargs) -> None:
...
def convert_tree(beautiful_soup_tree, makeelement=None) -> None:
...
def _parse(source, beautifulsoup, makeelement, **bsargs) -> None:
...
def _init_node_converters(makeelement) -> None:
    converters = {}
    ordered_node_types = []

    def converter(*types) -> None:
    ...
    def find_best_converter(node) -> None:
        for t in ordered_node_types:
            if isinstance(node, t):
                return converters[t]
        return None

    def convert_node(bs_node, parent=None) -> None:
    ...
    def append_text(parent, text) -> None:
        if len(parent) == 0:
    ...
    def convert_tag(bs_node, parent) -> None:
        attrs = bs_node.attrs
    ...
    @converter(Comment)
    def convert_comment(bs_node, parent) -> None:
    ...
    def convert_pi(bs_node, parent) -> None:
        if bs_node.endswith('?'):
    ...
    def convert_text(bs_node, parent) -> None:
        if parent is not None:
...
def unescape(string) -> None:
    if not string:
        return ''
    # work around oddities in BeautifulSoup's entity handling
    def unescape_entity(m) -> None:
        try:
