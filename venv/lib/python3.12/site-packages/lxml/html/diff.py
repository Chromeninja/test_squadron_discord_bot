...
def default_markup(text, version) -> None:
    return '<span title="%s">%s</span>' % (
        html_escape(_unicode(version), 1), text)

def html_annotate(doclist, markup=default_markup) -> None:
    """
...
def tokenize_annotated(doc, annotation) -> None: 
...
def compress_tokens(tokens) -> None:
    """
...
def markup_serialize_tokens(tokens, markup_func) -> None:
    """
...
def htmldiff(old_html, new_html) -> None:
...
def htmldiff_tokens(html1_tokens, html2_tokens) -> None:
...
def expand_tokens(tokens, equal=False) -> None:
...
def cleanup_delete(chunks) -> None:
...
def split_unbalanced(chunks) -> None:
...
def tokenize(html, include_hrefs=True) -> None:
    """
...
def parse_html(html, cleanup=True) -> None:
    """
...
def split_trailing_whitespace(word) -> None:
    """
...
def flatten_el(el, include_hrefs, skip_tag=False) -> None:
...
def is_word(tok) -> None:
    return not tok.startswith('<')

def is_end_tag(tok) -> None:
    return tok.startswith('</')

def is_start_tag(tok) -> None:
    return tok.startswith('<') and not tok.startswith('</')

def fixup_ins_del_tags(html) -> None:
...
def serialize_html_fragment(el, skip_outer=False) -> None:
...
def _fixup_ins_del_tags(doc) -> None:
...
def _move_el_inside_block(el, tag) -> None:
...
def _merge_element_contents(el) -> None:
    """
...
class InsensitiveSequenceMatcher(difflib.SequenceMatcher):
    ...
    def get_matching_blocks(self) -> None:
