...
def xpath_tokenizer(pattern, namespaces=None) -> None:
...
def prepare_child(next, token) -> None:
    tag = token[1]
    def select(result) -> None:
        for elem in result:
            for e in elem.iterchildren(tag):
                yield e
    return select

def prepare_star(next, token) -> None:
    def select(result) -> None:
        for elem in result:
...
def prepare_self(next, token) -> None:
    def select(result) -> None:
        return result
    return select

def prepare_descendant(next, token) -> None:
    token = next()
    if token[0] == "*":
        tag = "*"
    elif not token[0]:
...
def prepare_parent(next, token) -> None:
    def select(result) -> None:
        for elem in result:
...
def prepare_predicate(next, token) -> None:
...
def _build_path_iterator(path, namespaces) -> None:
...
def iterfind(elem, path, namespaces=None) -> None:
...
def find(elem, path, namespaces=None) -> None:
...
def findall(elem, path, namespaces=None) -> None:
...
def findtext(elem, path, default=None, namespaces=None) -> None:
