...
class LXMLOutputChecker(OutputChecker):
    ...
    def get_default_parser(self) -> None:
        return etree.XML
    ...
    def get_parser(self, want, got, optionflags) -> None: parser | None = None
    ...
    def _looks_like_markup(self, s) -> None:
        s = s.strip()
        return (s.startswith('<')
                and not _repr_re.search(s))

    def compare_docs(self, want, got) -> None:
    ...
    def text_compare(self, want, got, strip) -> None:
        want = want or ''
        got = got or ''
        if strip:
    ...
    def tag_compare(self, want, got) -> None:
        if want == 'any':
            return True
    ...
    def output_difference(self, example, got, optionflags) -> None:
        want = example.want
        parser = self.get_parser(want, got, optionflags)
        errors = []
    ...
    def html_empty_tag(self, el, html=True) -> None:
        if not html:
            return False
    ...
    def format_doc(self, doc, html, indent, prefix='') -> None:
        parts = []
        if not len(doc):
            # No children...
            parts.append(' '*indent)
    ...
    def format_text(self, text, strip=True) -> None:
        if text is None:
            return ''
        if strip:
            text = text.strip()
    ...
    def format_end_tag(self, el) -> None:
        if isinstance(el, etree.CommentBase):
            # FIXME: probably PIs should be handled specially too?
            return '-->'
        return '</%s>' % el.tag

    def collect_diff(self, want, got, html, indent) -> None:
        parts = []
    ...
    def collect_diff_tag(self, want, got) -> None:
        if not self.tag_compare(want.tag, got.tag):
    ...
    def collect_diff_end_tag(self, want, got) -> None:
        if want.tag != got.tag:
            tag = '%s (got: %s)' % (want.tag, got.tag)
        else:
            tag = got.tag
    ...
    def collect_diff_text(self, want, got, strip=True) -> None:
...
def temp_install(html=False, del_module=None) -> None:
    """
...
class _RestoreChecker(object):
    ...
    def __call__(self, *args, **kw) -> None:
        ...
        return result
    def call_super(self, *args, **kw) -> None:
        self.uninstall_clone()
        try:
