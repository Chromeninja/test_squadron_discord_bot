...
def fix_switch_cases(switch_node) -> None:
...
def _extract_nested_case(case_node, stmts_list) -> None:
...
def fix_atomic_specifiers(decl) -> None:
...
def _fix_atomic_specifiers_once(decl) -> None:
