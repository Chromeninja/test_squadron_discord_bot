...
def _parse_options(o_strs) -> None:
    opts = {}
    if not o_strs:
        return opts
    for o_str in o_strs:
...
def _parse_filters(f_strs) -> None:
    filters = []
    if not f_strs:
        return filters
    for f_str in f_strs:
...
def _print_help(what, name) -> None:
    try:
        if what == 'lexer':
...
def _print_list_as_json(requested_items) -> None:
    import json
    result = {}
...
def main_inner(parser, argns) -> None:
    if argns.help:
        parser.print_help()
        return 0
...
class HelpFormatter(argparse.HelpFormatter):
    def __init__(self, prog, indent_increment=2, max_help_position=16, width=None) -> None:
        if width is None:
            try:
