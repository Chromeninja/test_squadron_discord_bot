...
class SchemeLexer(RegexLexer):
    ...
    def decimal_cb(self, match) -> None:
        if '.' in match.group():
...
class HyLexer(RegexLexer):
    ...
    def _multi_escape(entries) -> None:
...
class ShenLexer(RegexLexer):
    ...
    def _relevant(self, token) -> None:
        return token not in (Text, Whitespace, Comment.Single, Comment.Multiline)

    def _process_declarations(self, tokens) -> None:
        opening_paren = False
    ...
    def _process_symbols(self, tokens) -> None:
        opening_paren = False
        for index, token, value in tokens:
    ...
    def _process_declaration(self, declaration, tokens) -> None:
        for index, token, value in tokens:
            if self._relevant(token):
                break
            yield index, token, value
    ...
    def _process_signature(self, tokens) -> None:
        for index, token, value in tokens:
