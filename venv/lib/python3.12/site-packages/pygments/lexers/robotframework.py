...
def normalize(string, remove='') -> None:
    string = string.lower()
    for char in remove + ' ':
        if char in string:
...
class VariableTokenizer:
    ...
    def tokenize(self, string, token) -> None:
    ...
    def _tokenize(self, var, string, orig_token) -> None:
...
class RowTokenizer:
    ...
    def _start_table(self, header) -> None:
        name = normalize(header, remove='*')
        return self._tables.get(name, UnknownTable())

    def _tokenize(self, value, index, commented, separator, heading) -> None:
        if commented:
            yield value, COMMENT
        elif separator:
...
class RowSplitter:
    ...
    def split(self, row) -> None:
        splitter = (row.startswith('| ') and self._split_from_pipes
                    or self._split_from_spaces)
        yield from splitter(row)
        yield '\n'

    def _split_from_spaces(self, row) -> None:
        yield ''  # Start with (pseudo)separator similarly as with pipes
        yield from self._space_splitter.split(row)

    def _split_from_pipes(self, row) -> None:
...
class Tokenizer: _tokens | None = None

    def __init__(self) -> None:
        self._index = 0

    def tokenize(self, value) -> None:
    ...
    def _tokenize(self, value, index) -> None:
        index = min(index, len(self._tokens) - 1)
        return self._tokens[index]

    def _is_assign(self, value) -> None:
        if value.endswith('='):
            value = value[:-1].strip()
...
class Setting(Tokenizer):
    ...
    def __init__(self, template_setter=None) -> None:
...
class KeywordCall(Tokenizer):
    ...
    def __init__(self, support_assign=True) -> None:
        Tokenizer.__init__(self)
...
class GherkinTokenizer:
    ...
    def tokenize(self, value, token) -> None:
...
class _Table: _tokenizer_class | None = None

    def __init__(self, prev_tokenizer=None) -> None:
        self._tokenizer = self._tokenizer_class()
        self._prev_tokenizer = prev_tokenizer
        self._prev_values_on_row = []

    def tokenize(self, value, index) -> None:
...
class SettingTable(_Table):
    ...
    def __init__(self, template_setter, prev_tokenizer=None) -> None:
...
class TestCaseTable(_Table):
    ...
    @property
    def _tokenizer_class(self) -> None:
    ...
    def _tokenize(self, value, index) -> None:
        if index == 0:
            if value: self._test_template | None = None
    ...
    def _is_setting(self, value) -> None:
        return value.startswith('[') and value.endswith(']')

    def _is_template(self, value) -> None:
        return normalize(value) == '[template]'

    def _is_for_loop(self, value) -> None:
    ...
    def _is_template_set(self, template) -> None:
...
class VariableSplitter:
    ...
    def __init__(self, string, identifiers) -> None: self.identifier | None = None
        self.base = None
        self.index = None
        self.start = -1
        self.end = -1
    ...
    def get_replaced_base(self, variables) -> None:
    ...
    def _scanning_list_variable_index(self) -> None:
        return self._state in [self._waiting_list_variable_index_state,
                               self._list_variable_index_state]

    def _find_variable(self, string) -> None:
    ...
    def _find_start_index(self, string, start, end) -> None:
    ...
    def _start_index_is_ok(self, string, index) -> None:
        return string[index] in self._identifiers\
        and not self._is_escaped(string, index)

    def _is_escaped(self, string, index) -> None:
        escaped = False
    ...
    def _variable_state(self, char, index) -> None:
    ...
    def _is_list_or_dict_variable(self) -> None:
        return self._variable_chars[0] in ('@','&')

    def _internal_variable_start_state(self, char, index) -> None:
        self._state = self._variable_state
        if char == '{':
            self._variable_chars.append(char)
            self._open_curly += 1
    ...
    def _waiting_list_variable_index_state(self, char, index) -> None:
        if char != '[':
            raise StopIteration
        self._list_and_dict_variable_index_chars.append(char)
        self._state = self._list_variable_index_state

    def _list_variable_index_state(self, char, index) -> None:
        self._list_and_dict_variable_index_chars.append(char)
        if char == ']':
            raise StopIteration
