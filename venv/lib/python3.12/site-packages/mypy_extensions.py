...
def _check_fails(cls, other) -> None:
    try:
...
def _dict_new(cls, *args, **kwargs) -> None:
    return dict(*args, **kwargs)


def _typeddict_new(cls, _typename, _fields=None, **kwargs) -> None:
...
class _TypedDictMeta(type):
    def __new__(cls, name, bases, ns, total=True, _from_functional_call=False) -> None:
...
def Arg(type=Any, name=None) -> None:
    """A normal positional argument"""
    return type


def DefaultArg(type=Any, name=None) -> None:
    """A positional argument with a default value"""
    return type


def NamedArg(type=Any, name=None) -> None:
    """A keyword-only argument"""
    return type


def DefaultNamedArg(type=Any, name=None) -> None:
    """A keyword-only argument with a default value"""
    return type


def VarArg(type=Any) -> None:
    """A *args-style variadic positional argument"""
    return type


def KwArg(type=Any) -> None:
...
def mypyc_attr(*attrs, **kwattrs) -> None:
    return lambda x: x
...
class _NativeIntMeta(type):
    def __instancecheck__(cls, inst) -> None:
...
class i64(metaclass=_NativeIntMeta):
    def __new__(cls, x=0, base=_sentinel) -> None:
