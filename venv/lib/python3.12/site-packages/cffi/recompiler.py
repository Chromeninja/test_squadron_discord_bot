...
class GlobalExpr:
    def __init__(self, name, address, type_op, size=0, check_value=0) -> None:
        self.name = name
...
class FieldExpr:
    ...
    def as_field_python_expr(self) -> None:
...
class StructUnionExpr:
    def __init__(self, name, type_index, flags, size, alignment, comment,
                 first_field_index, c_fields) -> None:
        self.name = name
...
class Recompiler:
    ...
    def __init__(self, ffi, module_name, target_is_python=False) -> None:
        self.ffi = ffi
    ...
    def _do_collect_type(self, tp) -> None:
    ...
    def _generate(self, step_name) -> None:
    ...
    def _rel_readlines(self, filename) -> None:
    ...
    def _gettypenum(self, type) -> None:
        # a KeyError here is a bug.  please report it! :-)
        return self._typesdict[type]

    def _convert_funcarg_to_c(self, tp, fromvar, tovar, errcode) -> None:
        extraarg = ''
    ...
    def _extra_local_variables(self, tp, localvars, freelines) -> None:
    ...
    def _convert_funcarg_to_c_ptr_or_array(self, tp, fromvar, tovar, errcode) -> None:
    ...
    def _convert_expr_from_c(self, tp, var, context) -> None:
    ...
    def _typedef_type(self, tp, name) -> None:
        return self._global_type(tp, "(*(%s *)0)" % (name,))

    def _generate_cpy_typedef_collecttype(self, tp, name) -> None:
        self._do_collect_type(self._typedef_type(tp, name))

    def _generate_cpy_typedef_decl(self, tp, name) -> None:
        pass

    def _typedef_ctx(self, tp, name) -> None:
        type_index = self._typesdict[tp]
        self._lsts["typename"].append(TypenameExpr(name, type_index))

    def _generate_cpy_typedef_ctx(self, tp, name) -> None:
    ...
    def _generate_cpy_function_collecttype(self, tp, name) -> None:
        self._do_collect_type(tp.as_raw_function())
        if tp.ellipsis and not self.target_is_python:
            self._do_collect_type(tp)

    def _generate_cpy_function_decl(self, tp, name) -> None:
        ...
        def need_indirection(type) -> None:
    ...
    def _generate_cpy_function_ctx(self, tp, name) -> None:
        if tp.ellipsis and not self.target_is_python:
    ...
    def _field_type(self, tp_struct, field_name, tp_field) -> None:
    ...
    def _struct_collecttype(self, tp) -> None:
        self._do_collect_type(tp)
        if self.target_is_python:
    ...
    def _struct_decl(self, tp, cname, approxname) -> None:
    ...
    def _struct_ctx(self, tp, cname, approxname, named_ptr=None) -> None:
        type_index = self._typesdict[tp]
    ...
    def _check_not_opaque(self, tp, location) -> None:
    ...
    def _add_missing_struct_unions(self) -> None:
    ...
    def _generate_cpy_struct_collecttype(self, tp, name) -> None:
        self._struct_collecttype(tp)
    _generate_cpy_union_collecttype = _generate_cpy_struct_collecttype

    def _struct_names(self, tp) -> None:
        cname = tp.get_c_name('')
        if ' ' in cname:
    ...
    def _generate_cpy_struct_decl(self, tp, name) -> None:
        self._struct_decl(tp, *self._struct_names(tp))
    _generate_cpy_union_decl = _generate_cpy_struct_decl

    def _generate_cpy_struct_ctx(self, tp, name) -> None:
    ...
    def _generate_cpy_anonymous_collecttype(self, tp, name) -> None:
        if isinstance(tp, model.EnumType):
            self._generate_cpy_enum_collecttype(tp, name)
        else:
            self._struct_collecttype(tp)

    def _generate_cpy_anonymous_decl(self, tp, name) -> None:
        if isinstance(tp, model.EnumType):
            self._generate_cpy_enum_decl(tp)
        else:
            self._struct_decl(tp, name, 'typedef_' + name)

    def _generate_cpy_anonymous_ctx(self, tp, name) -> None:
        if isinstance(tp, model.EnumType):
    ...
    def _generate_cpy_constant_collecttype(self, tp, name) -> None:
        is_int = tp.is_integer_type()
        if not is_int or self.target_is_python:
            self._do_collect_type(tp)

    def _generate_cpy_constant_decl(self, tp, name) -> None:
        is_int = tp.is_integer_type()
        self._generate_cpy_const(is_int, name, tp)

    def _generate_cpy_constant_ctx(self, tp, name) -> None:
    ...
    def _generate_cpy_enum_collecttype(self, tp, name) -> None:
        self._do_collect_type(tp)

    def _generate_cpy_enum_decl(self, tp, name=None) -> None:
        for enumerator in tp.enumerators:
            self._generate_cpy_const(True, enumerator)

    def _enum_ctx(self, tp, cname) -> None:
        type_index = self._typesdict[tp]
    ...
    def _generate_cpy_enum_ctx(self, tp, name) -> None:
        self._enum_ctx(tp, tp._get_c_name())

    # ----------
    # macros: for now only for integers

    def _generate_cpy_macro_collecttype(self, tp, name) -> None:
        pass

    def _generate_cpy_macro_decl(self, tp, name) -> None:
        if tp == '...':
            check_value = None
        else:
    ...
    def _generate_cpy_macro_ctx(self, tp, name) -> None:
        if tp == '...':
            if self.target_is_python:
                raise VerificationError(
    ...
    def _global_type(self, tp, global_name) -> None:
    ...
    def _generate_cpy_variable_collecttype(self, tp, name) -> None:
        self._do_collect_type(self._global_type(tp, name))

    def _generate_cpy_variable_decl(self, tp, name) -> None:
        prnt = self._prnt
    ...
    def _generate_cpy_variable_ctx(self, tp, name) -> None:
        tp = self._global_type(tp, name)
        type_index = self._typesdict[tp]
        if self.target_is_python:
            op = OP_GLOBAL_VAR
    ...
    def _generate_cpy_extern_python_collecttype(self, tp, name) -> None:
        assert isinstance(tp, model.FunctionPtrType)
        self._do_collect_type(tp)
    ...
    def _extern_python_decl(self, tp, name, tag_and_space) -> None:
        prnt = self._prnt
        if isinstance(tp.result, model.VoidType):
            size_of_result = '0'
        else:
    ...
    def _generate_cpy_extern_python_decl(self, tp, name) -> None:
        self._extern_python_decl(tp, name, 'static ')

    def _generate_cpy_dllexport_python_decl(self, tp, name) -> None:
        self._extern_python_decl(tp, name, 'CFFI_DLLEXPORT ')

    def _generate_cpy_extern_python_plus_c_decl(self, tp, name) -> None:
        self._extern_python_decl(tp, name, '')

    def _generate_cpy_extern_python_ctx(self, tp, name) -> None:
        if self.target_is_python:
            raise VerificationError(
    ...
    def _print_string_literal_in_array(self, s) -> None:
        prnt = self._prnt
    ...
    def _emit_bytecode_VoidType(self, tp, index) -> None:
        self.cffi_types[index] = CffiOp(OP_PRIMITIVE, PRIM_VOID)

    def _emit_bytecode_PrimitiveType(self, tp, index) -> None:
        prim_index = PRIMITIVE_TO_INDEX[tp.name]
        self.cffi_types[index] = CffiOp(OP_PRIMITIVE, prim_index)

    def _emit_bytecode_UnknownIntegerType(self, tp, index) -> None:
        s = ('_cffi_prim_int(sizeof(%s), (\n'
             '           ((%s)-1) | 0 /* check that %s is an integer type */\n'
             '         ) <= 0)' % (tp.name, tp.name, tp.name))
        self.cffi_types[index] = CffiOp(OP_PRIMITIVE, s)

    def _emit_bytecode_UnknownFloatType(self, tp, index) -> None:
        s = ('_cffi_prim_float(sizeof(%s) *\n'
             '           (((%s)1) / 2) * 2 /* integer => 0, float => 1 */\n'
             '         )' % (tp.name, tp.name))
        self.cffi_types[index] = CffiOp(OP_PRIMITIVE, s)

    def _emit_bytecode_RawFunctionType(self, tp, index) -> None:
    ...
    def _emit_bytecode_PointerType(self, tp, index) -> None:
    ...
    def _emit_bytecode_FunctionPtrType(self, tp, index) -> None:
        raw = tp.as_raw_function()
        self.cffi_types[index] = CffiOp(OP_POINTER, self._typesdict[raw])

    def _emit_bytecode_ArrayType(self, tp, index) -> None:
...
def _is_file_like(maybefile) -> None:
    # compare to xml.etree.ElementTree._get_writer
    return hasattr(maybefile, 'write')

def _make_c_or_py_source(ffi, module_name, preamble, target_file, verbose) -> None:
    if verbose:
...
def make_c_source(ffi, module_name, preamble, target_c_file, verbose=False) -> None:
    assert preamble is not None
    return _make_c_or_py_source(ffi, module_name, preamble, target_c_file,
                                verbose)

def make_py_source(ffi, module_name, target_py_file, verbose=False) -> None:
    return _make_c_or_py_source(ffi, module_name, None, target_py_file,
                                verbose)

def _modname_to_file(outputdir, modname, extension) -> None:
    parts = modname.split('.')
    try:
...
def _patch_meth(patchlist, cls, name, new_meth) -> None:
    old = getattr(cls, name)
    patchlist.append((cls, name, old))
    setattr(cls, name, new_meth)
    return old

def _unpatch_meths(patchlist) -> None:
    for cls, name, old_meth in reversed(patchlist):
        setattr(cls, name, old_meth)

def _patch_for_embedding(patchlist) -> None:
    ...
    if sys.platform == 'darwin':
        ...
        def my_link_shared_object(self, *args, **kwds) -> None:
...
def _patch_for_target(patchlist, target) -> None:
...
def recompile(ffi, module_name, preamble, tmpdir='.', call_c_compiler=True,
              ...
              uses_ffiplatform=True, **kwds) -> None:
