...
def pytest_fixture_setup(fixturedef) -> None:  # type: ignore[no-untyped-def]
    ...
    def wrapper(*args, **kwargs) -> None:  # type: ignore[no-untyped-def]
        ...
        if is_async_gen:
            ...
            def finalizer() -> None:  # type: ignore[no-untyped-def]
                try:
...
@pytest.fixture
def fast(request) -> None:  # type: ignore[no-untyped-def]
    """--fast config option"""
    return request.config.getoption("--aiohttp-fast")


@pytest.fixture
def loop_debug(request) -> None:  # type: ignore[no-untyped-def]
    """--enable-loop-debug config option"""
    return request.config.getoption("--aiohttp-enable-loop-debug")


@contextlib.contextmanager
def _runtime_warning_context() -> None:  # type: ignore[no-untyped-def]
...
@contextlib.contextmanager
def _passthrough_loop_context(loop, fast=False) -> None:  # type: ignore[no-untyped-def]
...
def pytest_pycollect_makeitem(collector, name, obj) -> None:  # type: ignore[no-untyped-def]
    """Fix pytest collecting for coroutines."""
    if collector.funcnamefilter(name) and inspect.iscoroutinefunction(obj):
        return list(collector._genfunctions(name, obj))


def pytest_pyfunc_call(pyfuncitem) -> None:  # type: ignore[no-untyped-def]
...
@pytest.fixture
def test_server(aiohttp_server) -> None:  # type: ignore[no-untyped-def]  # pragma: no cover
    warnings.warn(
...
@pytest.fixture
def test_client(aiohttp_client) -> None:  # type: ignore[no-untyped-def]  # pragma: no cover
    warnings.warn(
