...
class PerfTest(TestCase):
    ...
    @timed
    async def test_connection_memory(self) -> None:
        while True:
            yield
    ...
    @timed
    async def test_connection_file(self) -> None:
    ...
    @timed
    async def test_atomics(self) -> None:
    ...
    @timed
    async def test_inserts(self) -> None:
        async with aiosqlite.connect(TEST_DB) as db:
            await db.execute("create table perf (i integer primary key asc, k integer)")
            await db.commit()
    ...
    @timed
    async def test_insert_ids(self) -> None:
        async with aiosqlite.connect(TEST_DB) as db:
            await db.execute("create table perf (i integer primary key asc, k integer)")
            await db.commit()
    ...
    @timed
    async def test_insert_macro_ids(self) -> None:
        async with aiosqlite.connect(TEST_DB) as db:
            await db.execute("create table perf (i integer primary key asc, k integer)")
            await db.commit()
    ...
    @timed
    async def test_select_macro(self) -> None:
        async with aiosqlite.connect(TEST_DB) as db:
            await db.execute("create table perf (i integer primary key asc, k integer)")
            for i in range(100):
                await db.execute("insert into perf (k) values (%d)" % (i,))
    ...
    async def test_iterable_cursor_perf(self) -> None:
        async with aiosqlite.connect(TEST_DB) as db:
            await db.execute(
            ...
            async def test_perf(chunk_size: int) -> None:
                while True:
