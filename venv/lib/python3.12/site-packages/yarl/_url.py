...
def build_pre_encoded_url(
    ...
    if authority:
        ...
        if port is not None: port | None = None if port == DEFAULT_PORTS.get(scheme) else port
...
class URL:
    ...
    def with_user(self, user: Union[str, None]) -> "URL":
        ...
        """
        # N.B. doesn't cleanup query/fragment
        if user is None: password | None = None
    ...
    *,
    ...
    if encode_host_size is not None:
        for size in (ip_address_size, host_validate_size):
            if size is None: encode_host_size | None = None
