...
class Emitter:
    ...
    def __init__(self, stream, canonical=None, indent=None, width=None,
            allow_unicode=None, line_break=None) -> None:
    ...
    def need_more_events(self) -> None:
        if not self.events:
            return True
        event = self.events[0]
    ...
    def need_events(self, count) -> None:
        level = 0
    ...
    def expect_document_start(self, first=False) -> None:
    ...
    def expect_block_sequence_item(self, first=False) -> None:
    ...
    def expect_block_mapping_key(self, first=False) -> None:
    ...
    def check_empty_sequence(self) -> None:
        return (isinstance(self.event, SequenceStartEvent) and self.events
                and isinstance(self.events[0], SequenceEndEvent))

    def check_empty_mapping(self) -> None:
        return (isinstance(self.event, MappingStartEvent) and self.events
                and isinstance(self.events[0], MappingEndEvent))

    def check_empty_document(self) -> None:
    ...
    def check_simple_key(self) -> None:
        length = 0
    ...
    def process_anchor(self, indicator) -> None:
        if self.event.anchor is None: self.prepared_anchor | None = None
            return
        if self.prepared_anchor is None:
            self.prepared_anchor = self.prepare_anchor(self.event.anchor)
    ...
    def process_tag(self) -> None:
        ...
        if isinstance(self.event, ScalarEvent):
            ...
            if (not self.canonical or tag is None) and self.event.implicit: self.prepared_tag | None = None
                return
        if tag is None:
    ...
    def choose_scalar_style(self) -> None:
        if self.analysis is None:
            self.analysis = self.analyze_scalar(self.event.value)
    ...
    def prepare_version(self, version) -> None:
        major, minor = version
        if major != 1:
            raise EmitterError("unsupported YAML version: %d.%d" % (major, minor))
        return '%d.%d' % (major, minor)

    def prepare_tag_handle(self, handle) -> None:
        if not handle:
    ...
    def prepare_tag_prefix(self, prefix) -> None:
        if not prefix:
    ...
    def prepare_anchor(self, anchor) -> None:
        if not anchor:
    ...
    def analyze_scalar(self, scalar) -> None:
    ...
    def write_indicator(self, indicator, need_whitespace,
            whitespace=False, indention=False) -> None:
    ...
    def write_single_quoted(self, text, split=True) -> None:
        self.write_indicator('\'', True)
        spaces = False
        breaks = False
        start = end = 0
    ...
    def write_double_quoted(self, text, split=True) -> None:
        self.write_indicator('"', True)
        start = end = 0
        while end <= len(text):
            ch = None
    ...
    def determine_block_hints(self, text) -> None:
        hints = ''
        if text:
            if text[0] in ' \n\x85\u2028\u2029':
    ...
    def write_plain(self, text, split=True) -> None:
